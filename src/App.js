...
import { QUESTION_LIMIT, OPERATORS } from "./utils/miscUtils";
...
const App = () => {
  ...
  const _generateWrongOptions = (ans) => {
    let num = Math.floor(Math.random() * 90);
    return (num === ans)
      ? _generateWrongOptions(ans)
      : {
        value: num,
        correct: false,
        clicked: false
  ...
  const generateQuestion = (name) => {
    ...
    if (isFinite(eval(`${operand1 + selectedOperator + operand2}`))) {
      ...
      while (ans.length < 4) 
        ans.push(_generateWrongOptions(eval(`${operand1 + selectedOperator + operand2}`)));

      setActiveQuestion({
        ...activeQuestion,
        [name]: {
          question: `${operand1 + selectedOperator + operand2}`,
          options: _shuffleArray(ans)
        }
      })
  ...
  const handleOptionClick = (question, options, option, name) => {
    ...
    if (allQuestions[name].length === QUESTION_LIMIT) {
